...
- The repository-linked [Nardora Roadmap Project](https://github.com/users/MetinGames/projects/2) now contains every open Issue with Active Work and Blocked views, Status/Priority/Phase metadata, and Phase 0/Phase 1/Hosted Alpha milestones. Effort and Target Window columns remain the final [#8](https://github.com/MetinGames/Long-Narde/issues/8) completion fields.
...
| Hosted evidence gate | [#20](https://github.com/MetinGames/Long-Narde/issues/20) hosted private table | Authenticated two-browser flow, authoritative gameplay, abuse/load, physical-device, production-region, privacy, and launch approval | Keep the verified Free-plan synthetic provider boundary and Friend Match/Online disabled; do not collect real-player data |
...
| Owner setting nearly complete | [#8](https://github.com/MetinGames/Long-Narde/issues/8) Project workflow | Add and populate Effort and Target Window; correct #67 to Phase 2 | Preserve the linked Project, Active Work/Blocked filters, milestones, labels, catalog, roadmap checks, and workflow documentation |
| Evidence pending | [#41](https://github.com/MetinGames/Long-Narde/issues/41) research-pool closure | Real-player, physical-device, original-audio, native-Russian, and hosted-provider evidence | Keep the completed research decisions and bounded child Issues synchronized |
| Evidence pending | [#67](https://github.com/MetinGames/Long-Narde/issues/67) physical haptic validation | Android Chrome/PWA and iPhone Safari/PWA feel, interruption, and accessibility notes | Keep the default-off adapter and deterministic regressions green |
| Evidence pending | [#68](https://github.com/MetinGames/Long-Narde/issues/68) physical HiDPI validation | Retina iPhone and representative Android screenshot/diagnostic evidence | Keep DPR geometry, inverse touch mapping, and mobile-browser regressions green |
| No approval needed | Local quality and maintenance | None within the accepted scope | Continue tests, dependency review, security checks, and factual roadmap synchronization |
...
9. **Engineering complete; player-evidence gate remains — [#11](https://github.com/MetinGames/Long-Narde/issues/11):** the deterministic baseline, profiling, request-scoped cache, opponent-aware fixtures, fixed 16-seed/32-match validation, symmetric player-two pip correction, and deterministic opponent-prime threat fixture are complete. The general threat refinement preserves the 31–1 sample while improving the sole loss from 2/15 to 7/15 collected checkers and from 29 to 11 remaining pips; only new real-match evidence can justify another strategy change.
10. **Physical-device gate remains — [#13](https://github.com/MetinGames/Long-Narde/issues/13):** the desktop-Chrome cycle is complete through [#40](https://github.com/MetinGames/Long-Narde/pull/40). Physical iPhone Safari evidence then exposed compact portrait control collisions, a blocking rotation notice, and an overflowing landscape dice card; [#43](https://github.com/MetinGames/Long-Narde/pull/43) adds deterministic portrait/landscape grids plus WebKit geometry regression coverage. Physical iPhone recheck, Android, and tablet validation remain open without changing Long Narde rules.
11. **Project workflow nearly complete — [#8](https://github.com/MetinGames/Long-Narde/issues/8) and completed [#6](https://github.com/MetinGames/Long-Narde/issues/6):** the repository-linked Nardora Roadmap Project contains all eight open Issues, Active Work/Blocked filters, Status/Priority/Phase fields, and Phase 0/Phase 1/Hosted Alpha milestones. Effort and Target Window remain before #8 closes. Merged [#36](https://github.com/MetinGames/Long-Narde/pull/36) records the complete 50-branch inventory with 45 exact cleanup candidates, one active branch, and four preserved for manual review. No branch has been deleted.
12. **Hosted evidence gate:** [#20](https://github.com/MetinGames/Long-Narde/issues/20) now has a healthy Free-plan Frankfurt synthetic provider, managed Realtime, an active JWT-required Edge Function, a verified revisions 1–9 lifecycle, zero retained trial rows, and zero Security Advisor lints. Authenticated two-browser, authoritative gameplay, abuse/load, physical-device, production-region, privacy, and launch gates remain; Friend Match/Online stay disabled.
...
22. **Engineering complete; recording/device gate remains — [#12](https://github.com/MetinGames/Long-Narde/issues/12):** added accessible persistent mute and master-volume controls, live Web Audio gain updates, TR/EN/RU labels, mobile styling, and PWA precache coverage. Original recordings and physical phone/desktop listening validation remain open.
...
5. **Hosted synthetic evidence — Verified; launch gated:** [#19](https://github.com/MetinGames/Long-Narde/issues/19) records provider/privacy/cost evidence. [#20](https://github.com/MetinGames/Long-Narde/issues/20) now proves a generated-data Supabase Free lifecycle through managed Realtime and a JWT-required Edge Function, with zero retained rows. Authenticated gameplay authority, abuse/load, physical-device, production-region, privacy, spend, and launch approval remain.
...
- **Hosted evidence gated:** Keep [#20](https://github.com/MetinGames/Long-Narde/issues/20) blocked after the successful Free-plan synthetic-provider proof until authenticated two-browser gameplay authority, abuse/load, physical-device, production-region, privacy, spend, and launch gates pass.
...
- **Engineering implemented; physical browser gate remains:** rebuilt the start menu around a board-first composition with one gold primary action, a single-line saved-match row, compact match settings and quieter inactive modes. Phone controls remain at least 44 px and short-landscape layouts compact without changing game behavior; Chrome desktop and iPhone 16e Safari visual rechecks remain before [#81](https://github.com/MetinGames/Long-Narde/issues/81) can close. Active-game reflow remains separately tracked in [#79](https://github.com/MetinGames/Long-Narde/issues/79).
