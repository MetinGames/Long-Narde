...
test('undo sonrasi yasal hamle kalmadiginda confirm gorunur ve aktif olur', () => {
    const syntheticGame = {
        currentPlayer: 1,
        gameStatus: 'PLAYING',
...
test('undo sonrasi yasal hamle varsa undo aktif ve confirm kurala uygun pasif olur', () => {
    const syntheticGame = {
        currentPlayer: 1,
        gameStatus: 'PLAYING',
        moveHistory: [{ snapshot: true }],
...
test('cift zarda art arda iki undo dogru calisir', () => {
    const game = prepareGame({
        dice: [4, 4, 4, 4],
        pieces: [{ slot: 2, count: 1, owner: 1 }]
    });
...
test('gecersiz undo hicbir ui durumunu bozmaz', () => {
    const game = prepareGame({
        dice: [5],
        pieces: [{ slot: 2, count: 1, owner: 1 }]
    });
