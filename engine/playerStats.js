...
function sanitizePlayerStats(raw) {
    ...
    if (stats.wins > stats.totalMatches) {
        stats.wins = stats.totalMatches;
    }

    if (stats.losses > stats.totalMatches) {
        stats.losses = stats.totalMatches;
    }

    const totalOutcomes = stats.wins + stats.losses;
    if (totalOutcomes !== stats.totalMatches) {
...
export class PlayerStatsStore {
    ...
    save(stats) {
        ...
        try {
            ...
            // Storage quota or access issues should not break gameplay.
        }
